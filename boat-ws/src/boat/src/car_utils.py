...
class imgproc:
	...
	def get_color_mask(self,image,color_range):
		masks = []
		for x in color_range:
			lower = np.array(x[0],dtype="uint8")
			upper = np.array(x[1],dtype="uint8")
			masks += [cv2.inRange(cv2.cvtColor(image,cv2.COLOR_BGR2HSV), lower, upper)]
		combined = sum(masks)
	...
	def get_shapes(self,color_ranges):
		if self.display:
			overlay = self.image.copy()	# Make a copy of the image data (usually ZED output) unaltered
		Centers = {}	# Soon-to-be X value of contour centroids as average distance from center (- = left, + = right)
		for z in range(len(color_ranges)):	# Can recognize objects of multiple color ranges,
		#these will be the integer keys to Centers while a list of the centroid X values of contours of that color will be the values
			L_Centers = []
			R_Centers = []
			clean = self.clean_image(color_ranges[z])
			threshold = cv2.threshold(clean, 0, 255, 0)[1]
			contours = cv2.findContours(threshold, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)[1]
			for c in contours:
				if cv2.contourArea(c) >= 500:
					imageMoment = cv2.moments(c)
					...
					if self.display:
						cv2.circle(overlay, (centerX, centerY), 5, (0, 0, 255), -1)
			common = min([len(L_Centers),len(R_Centers)])	# Do not include contours seen only by one "eye"
			if common >= 1:
				for i in range(common):
					if z not in Centers:
						Centers[z] = [(L_Centers[i] + R_Centers[i] - self.width)/15]	# Don't ask my why 15 gets a near-perfect conversion to angle-off-center
					else:
						Centers[z] += [(L_Centers[i] + R_Centers[i] - self.width)/15]	# It just does
		if self.display:
			try:
				self.web_view.publish(bridge.cv2_to_imgmsg(self.get_ROI_mask(overlay,crop=False), "bgr8"))
			except CvBridgeError as e:
